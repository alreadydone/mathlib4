...
variable [Field K] {L : Type*} [Field L] [Algebra A K] [IsFractionRing A K] {g : A →+* L}
...
variable [CommRing K] [Algebra A K] [IsFractionRing A K] {L : Type*} [Field L] {g : A →+* L}

theorem mk'_eq_zero_iff_eq_zero {x : A} {y : nonZeroDivisors A} : mk' K x y = 0 ↔ x = 0 := by
  simp [mk'_eq_zero_iff]

theorem mk'_eq_one_iff_eq {x : A} {y : nonZeroDivisors A} : mk' K x y = 1 ↔ x = y := by
  simp [mk'_eq_iff_eq_mul]
...
theorem ringHom_ext {f1 f2 : K →+* P}
    (hf : ∀ x : A, f1 (algebraMap A K x) = f2 (algebraMap A K x)) : f1 = f2 :=
  IsLocalization.ringHom_ext (nonZeroDivisors A) (RingHom.ext hf)

theorem injective_comp_algebraMap :
    Function.Injective fun (f : K →+* P) => f.comp (algebraMap A K) :=
...
theorem lift_mk' (hg : Injective g) (x) (y : nonZeroDivisors A) :
    lift hg (mk' K x y) = g x / g y := (lift_mk'_spec ..).mpr <| by
  have := g.domain_nontrivial
  rw [mul_div_cancel₀ _ (map_ne_zero_of_mem_nonZeroDivisors g hg y.2)]
...
variable {A B C D : Type*}
  ...
  (FA FB FC FD : Type*) [CommRing FA] [CommRing FB] [CommRing FC] [CommRing FD]
...
noncomputable def fieldEquivOfAlgEquiv (f : B ≃ₐ[A] C) : FB ≃ₐ[FA] FC where
  ...
  commutes' x :=
    have : (ringEquivOfRingEquiv f.toRingEquiv).toRingHom.comp (algebraMap FA FB) =
        algebraMap FA FC := ringHom_ext fun x : A ↦ by
      simp_rw [RingHom.comp_apply, ← IsScalarTower.algebraMap_apply,
...
lemma fieldEquivOfAlgEquiv_trans (f : B ≃ₐ[A] C) (g : C ≃ₐ[A] D) :
  ...
  apply_fun AlgEquiv.toAlgHom
  on_goal 2 => intro _ _ h; ext; exact congr($h _)
  apply_fun AlgHom.toRingHom
  on_goal 2 => intro _ _ h; ext; exact congr($h _)
  exact ringHom_ext fun x : B ↦ by simp
...
theorem isFractionRing_iff_of_base_ringEquiv (h : R ≃+* A) :
    IsFractionRing R S ↔
      @IsFractionRing A _ S _ ((algebraMap R S).comp h.symm.toRingHom).toAlgebra := by
